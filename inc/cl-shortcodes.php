 ...
 */
function uri_cl_get_media_id_from_url( $media_url = '' ) {
	...
	if ( false !== strpos( $media_url, $upload_dir_paths['baseurl'] ) ) {

		// If this is the URL of an auto-generated thumbnail, get the URL of the original image
		$media_url = preg_replace( '/-\d+x\d+(?=\.(jpg|jpeg|png|gif)$)/i', '', $media_url );

		// Remove the upload path base directory from the attachment URL
		$media_url = str_replace( $upload_dir_paths['baseurl'] . '/', '', $media_url );

		// Finally, run a custom database query to get the attachment ID from the modified attachment URL
		$media_id = $wpdb->get_var( $wpdb->prepare( "SELECT wposts.ID FROM $wpdb->posts wposts, $wpdb->postmeta wpostmeta WHERE wposts.ID = wpostmeta.post_id AND wpostmeta.meta_key = '_wp_attached_file' AND wpostmeta.meta_value = %s AND wposts.post_type = 'attachment'", $media_url ) );
